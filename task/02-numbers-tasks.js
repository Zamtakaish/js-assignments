...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let width  = Math.abs(0 - x1);
    let height = Math.abs(0 - y1);
    let absA = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
    width  = Math.abs(0 - x2);
    height = Math.abs(0 - y2);
    let absB = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
    return Math.acos(dotProduct/(absA*absB));
}
...
function roundToPowerOfTen(num, pow) {
    ...
    if ((pow <= arr.length) && (pow !== 0) && (arr[pow - 1] > 5)){
        arr[pow]++;
    }
    for (let i = 0; i < pow; i++){
        arr[i] = 0;
